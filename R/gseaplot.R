    ...
    if (by == "runningScore" || by == "all") {
        ...
        p.res <- p.res + theme(legend.position = "none") +
            annotation_custom(tp,
                              xmin = quantile(p.res$data$x, .4),#修改了一下表格左边界
                              xmax = quantile(p.res$data$x, .95),
                              ymin = quantile(p.res$data$runningScore, .65),#修改了一下表格下边界
...
##' @author Guangchuang Yu
gseaplot2 <- function(x, geneSetID, title = "", color="#FA5860", base_size = 12, #修改一下颜色和基础尺寸
    ...
    p.res <- p.res + ylab("Running Enrichment Score") +
        theme(axis.text.x=element_blank(),
              ...
              geom_hline(yintercept = 0, linetype = "dashed") #添加ES为0的基线
    ...
    p2 <- ggplot(gsdata, aes_(x = ~x)) +
        ...
        xlab(NULL) + ylab(NULL) + theme_bw(base_size) +
    ...
    df2$y <- p$data$geneList[df2$x]
    p.pos <- p + geom_segment(data=df2, aes_(x=~x, xend=~x, y=~y, yend=0,color=~y),
                              show.legend = F)+
                              scale_colour_gradient2(low = "#2874C5", high = "#f87669")
    p.pos <- p.pos + ylab("Ranked List Metric") +
    ...
    if (pvalue_table) {
        ...
        if (colnames(pd[4]) == "qvalue") {
            colnames(pd[4]) <- "qvalues"
        }
...
##' @export
##' @author Guangchuang Yu
gsearank <- function(x, geneSetID, title="", pvalue_table = FALSE,output = "plot") {
    ...
    p<- p+ theme(legend.position = "none") +
        annotation_custom(tp,
                          xmin = quantile(p$data$x, .75),
                          xmax = quantile(p$data$x, .95),
                          ymin = quantile(p$data$runningScore, .75),
                          ymax = quantile(p$data$runningScore, .9))
    }
    return(p)
}
